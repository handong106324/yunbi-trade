...
public class TendencyResult {
    ...
    public int getBestTend() {
        int tend = oneUpTimes/oneDownTimes;
        int temp = twoUpTimes/twoDownTime;
        if(temp > tend) {
            tend = temp;
        }

        temp = threeDownTime > 0 ?threeUpTimes/threeDownTime:temp;

        if(temp > tend) {
            tend = temp;
        }

        temp = fourDownTime > 0 ?fourUpTimes/fourDownTime : temp;
        if(temp > tend) {
            tend = temp;
        }

        temp = fiveDownTime > 0 ?fiveUpTimes/fiveDownTime : temp;

        if(temp > tend) {
            tend = temp;
        }

        temp = sixDownTime > 0 ?sixUpTimes/sixDownTime : temp;
        if(temp > tend) {
            tend = temp;
        }
        temp = sevenDownTime > 0?sevenUpTimes/sevenDownTime : temp;

        if(temp > tend) {
            tend = temp;
        }
        temp = eightDownTime > 0 ?eightUpTimes/eightDownTime : temp;

        if(temp > tend) {
            tend = temp;
        }
        temp = nineDownTime > 0 ?nineUpTimes/nineDownTime : temp;
        if(temp > tend) {
            tend = temp;
        }
        temp = tenDownTime > 0 ?tenUpTimes/tenDownTime : temp;
        if(temp > tend) {
            tend = temp;
        }
